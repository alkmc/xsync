...
Upstream keys work by `string` and returns `any`, so every caller has to assert
the result type. Here both the key and the value are type parameters:

```go
var g singleflight.Group[int, *User]
...
It was parameterized over the key and value types in place of string keys and
`any` values, and its error handling was updated for Go 1.27. This repository
is not affiliated with or endorsed by Google or the Go project.
